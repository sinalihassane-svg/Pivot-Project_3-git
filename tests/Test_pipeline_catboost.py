...
Cancer du col de l'utérus · CatBoost Classifier
=============================================================================
Catégories de tests :
...
# ── Chemin du modèle ──────────────────────────────────────────────────────────
MODEL_PATH = os.path.join(os.path.dirname(__file__), '..', 'modeles', 'modele_catboost.pkl')
...
class TestModele:
    ...
    def test_nombre_itérations_catboost(self, model):
        """Vérifie le nombre d'itérations (arbres) du modèle CatBoost."""
        # Dans CatBoost, n_estimators est stocké dans tree_count_
        nb_arbres = model.tree_count_
        assert nb_arbres == 500, f"Nombre d'itérations incorrect : {nb_arbres}"

    def test_modele_catboost_entraine(self, model):
        """Vérifie si le modèle CatBoost a bien été entraîné."""
        # Pour CatBoost, on vérifie si l'attribut is_fitted_() est vrai
        assert model.is_fitted(), "Le modèle CatBoost n'est pas entraîné"

    def test_nombre_features_catboost(self, model):
        """Vérifie que le modèle CatBoost accepte le bon nombre de caractéristiques."""
        # CatBoost utilise n_features_in_ comme Scikit-Learn (depuis les versions récentes)
        # ou model.feature_names_ pour compter
        nb_features_model = len(model.feature_names_)
        assert nb_features_model == X_test_final.shape[1], \
            f"Nombre de features attendu ({nb_features_model}) ≠ disponibles ({X_test_final.shape[1]})"
